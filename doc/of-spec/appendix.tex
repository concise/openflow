...
\label{cts:qos}
\qosupd{An OpenFlow switch supports limited QoS through a simple queuing
mechanism. A queue is attached to a port and can be used to map flows
on it. Flows that are mapped to a specific queue inherit its
characteristics based on queue's configuration. 
\\\\
Configuration of a
queue takes place outside the OpenFlow protocol, either through the
command line or through an external dedicated configuration
protocol. 
\\\\
The controller can query the switch for configured queues
using the OFPQ\_GET\_CONFIG\_REQUEST :
\input{struct/ofp_queue_get__config_request}
The switch replies back with an OFPQ\_GET\_CONFIG\_REPLY, containing
a list of configured queues.
\input{struct/ofp_queue_get_config_reply}.
Each queue belongs to a discipline and is 
further described by an ofp\_queue structure:
\input{enum/ofp_queue_type}
\\\\
\input{struct/ofp_queue}
}
...
The \verb|max_len| indicates the maximum amount of data from a packet that should be sent when the port is \verb|OFPP_CONTROLLER|.  If \verb|max_len| is zero, the switch must send a zero-size \verb|packet_in| message.  The \verb|port| specifies the physical port from which packets should be sent. 
 \\\\
\qosupd{The enqueue action is used to map a flow to a queue, where we assume
  that the queue is already configured. ACTION\_ENQUEUE directly
  associates the flow with the specific queue and override any
  behavior that might be associated with TOS or VLAN\_PCP bits. 
  The packet should not change after an enqueue action. If the switch
  needs to set TOS/PCP bits for internal handling, the original values
...
\qosupd{\input{struct/ofp_action_enqueue}}
...
\qosupd{Information about queues is requested with OFPST\_QUEUE stats request
type. The request body consists of a uint16\_t port\_no field
identifying the port and a 64-bit identifier for the queue. OFPT\_NONE
refers to all ports, while UINT64\_MAX refers to all queues configured
at a port. 

...
\subsubsection{Neighbor Message}
As neighbor as discovered or disconnected, the controller needs to be informed via the \verb|OFPT_NEIGHBOR_MSG| message:

\input{struct/ofp_neighbor_msg}
The \verb|activity| is an enumeration that indicates if a new neighbor is found or an old neighbor is lost.  The \verb|max_miss_interval| field should be \verb|neighbor_probe_active_interval| times the maximum number of tolerable consecutive loss.

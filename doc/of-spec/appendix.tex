...
The \verb|cookie| field is an opaque data field that is set by the controller.  It is not used in any matching functions, and thus does not need to reside in hardware.  It is required that when \verb|command| is \verb|OFPC_MODIFY| or \verb|OFPC_MODIFY_STRICT| that matched flows have their \verb|cookie| field updated appropriately. 
\\\\

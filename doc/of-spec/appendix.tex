...
characteristics based on queue's configuration. Configuration of a
queue takes place outside the OpenFlow protocol, either through the
command line or through an external dedicated configuration protocol.}
...
\qosupd{The enqueue action is used to map a flow to a queue, where we assume
  ...
  The packet should not change after to an enqueue action. If the switch
